...
py::array_t<int> py_ecp_hale_t_2d3c(py::array_t<int> image_c1,
                                    py::array_t<int> image_c2,
                                    py::array_t<int> image_c3, int T1, int T2,
                                    int T3) {
    Matrix2d<int> img_c1 = pad_img_2d(image_c1, T1 + 1);
    Matrix2d<int> img_c2 = pad_img_2d(image_c2, T2 + 1);
    Matrix2d<int> img_c3 = pad_img_2d(image_c3, T3 + 1);
    ...
    for (size_t j = 1; j < num_cols; ++j) {
        // top-left vertex
        ecp(std::min({img_c1(num_rows, j), img_c1(num_rows - 1, j),
                      img_c1(num_rows - 1, j - 1), img_c1(num_rows, j - 1)}),
            std::min({img_c2(num_rows, j), img_c2(num_rows - 1, j),
                      img_c2(num_rows - 1, j - 1), img_c2(num_rows, j - 1)}),
            std::min({img_c3(num_rows, j), img_c3(num_rows - 1, j),
                      img_c3(num_rows - 1, j - 1), img_c3(num_rows, j - 1)})) +=
            1;
        ...
        ecp(std::min({img_c1(num_rows, j), img_c1(num_rows - 1, j)}),
            std::min({img_c2(num_rows, j), img_c2(num_rows - 1, j)}),
            std::min({img_c3(num_rows, j), img_c3(num_rows - 1, j)})) += -1;
    ...
    for (size_t i = 1; i < num_rows; ++i) {
        // top-left vertex
        ecp(std::min({img_c1(i, num_cols), img_c1(i - 1, num_cols),
                      img_c1(i - 1, num_cols - 1), img_c1(i, num_cols - 1)}),
            std::min({img_c2(i, num_cols), img_c2(i - 1, num_cols),
                      img_c2(i - 1, num_cols - 1), img_c2(i, num_cols - 1)}),
            std::min({img_c3(i, num_cols), img_c3(i - 1, num_cols),
                      img_c3(i - 1, num_cols - 1), img_c3(i, num_cols - 1)})) +=
            1;
        ...
        // left edge
        ecp(std::min({img_c1(i, num_cols), img_c1(i, num_cols - 1)}),
            std::min({img_c2(i, num_cols), img_c2(i, num_cols - 1)}),
            std::min({img_c3(i, num_cols), img_c3(i, num_cols - 1)})) += -1;
    ...
    // bottom-right pixel
    // top-left vertex
    ecp(std::min({img_c1(num_rows, num_cols), img_c1(num_rows - 1, num_cols),
                  img_c1(num_rows - 1, num_cols - 1),
                  img_c1(num_rows, num_cols - 1)}),
        std::min({img_c2(num_rows, num_cols), img_c2(num_rows - 1, num_cols),
                  img_c2(num_rows - 1, num_cols - 1),
                  img_c2(num_rows, num_cols - 1)}),
        std::min({img_c3(num_rows, num_cols), img_c3(num_rows - 1, num_cols),
                  img_c3(num_rows - 1, num_cols - 1),
                  img_c3(num_rows, num_cols - 1)})) += 1;
    // top-right vertex - is canceled by the top edge
    // bottom-right vertex - is canceled by the right edge
    // bottom-left vertex - is canceled by the left edge
    //
...
py::array_t<int> py_ecp_hale_v_2d3c(py::array_t<int> image_c1,
                                    py::array_t<int> image_c2,
                                    py::array_t<int> image_c3, int T1, int T2,
                                    int T3) {
    auto img_c1 = image_c1.unchecked<2>();
...
py::array_t<int> py_ecp_hale_i_2d3c(py::array_t<int> image_c1,
                                    py::array_t<int> image_c2,
                                    py::array_t<int> image_c3, int T1, int T2,
                                    int T3) {
    auto img_c1 = image_c1.unchecked<2>();
...
py::array_t<int> py_ecp_hale_eulearning_2d3c(py::array_t<int> contributions,
...
PYBIND11_MODULE(euprima, m) {
    ...
    m.def(
        "list_of_minimal_grades", &list_of_minimal_grades,
    ...
    m.def(
        "ecp_hale_eulearning_2d3c", &py_ecp_hale_eulearning_2d3c,
        "HALE:T(2,3) algorithm to compute the ECP of the filtration induced by "
        "top-cells of a 2-dimensional 3-channel image using precomputed list "
        "of minimal grades");
}
